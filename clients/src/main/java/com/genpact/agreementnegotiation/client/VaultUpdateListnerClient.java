 ...
 */
public class VaultUpdateListnerClient {
    private static final Logger logger = LoggerFactory.getLogger(VaultUpdateListnerClient.class);
    ...
    public static void main(String[] args) throws ActiveMQException, InterruptedException, ExecutionException {
        ...
        snapshot.getStates().forEach(VaultUpdateListnerClient::logState);
        updates.toBlocking().subscribe(update -> update.getProduced().forEach(VaultUpdateListnerClient::logState));
